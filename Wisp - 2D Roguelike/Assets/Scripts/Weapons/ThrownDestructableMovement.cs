...
public class ThrownDestructableMovement {
...
// When changing scenes (or Grid object) the grid of this component will have to be updated.
    ...
    public ThrownDestructableMovement(GameObject actor, MonoBehaviour mb, Grid grid) {
        this.actor = actor;
    ...
    private IEnumerator SmoothMovement (Vector3 end) {
        ...
        while (sqrRemainingDistance > float.Epsilon) {
            ...
            Vector3 newPostion = Vector3.MoveTowards(actorTransform.position, end, inverseMoveTime * Time.deltaTime);

            // Call MovePosition on attached Rigidbody2D and move it to the calculated position.
            actorTransform.position = newPostion;
...
/*
IEnumerator MoveToTarget(Vector3 start, Vector3 target){
  float t = 0f;
